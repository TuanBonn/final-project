...
// // Đã sửa lại CÚ PHÁP JOIN trong hàm GET
...
// // --- Hàm xác thực (Tái sử dụng) ---
// function getVerifiedUserIdFromToken(token: string | undefined): string | null {
//   if (!JWT_SECRET) {
//     console.error("[API Products] JWT_SECRET thiếu.");
//     return null;
//   }
//   if (!token) {
//     /* console.log("[API Products] Không có token."); */ return null;
//   }
//   try {
//     const payload = jwt.verify(token, JWT_SECRET) as JwtPayload;
//     return payload?.userId ?? null;
//   } catch (error) {
//     console.error("[API Products] Lỗi verify token:", error);
//     return null;
//   }
// }

// // === HÀM GET (ĐÃ SỬA CÚ PHÁP JOIN) ===
// export async function GET(request: Request) {
//   console.log("API GET /products: Bắt đầu (Bản vá lỗi Join V3)...");
//   try {
//     const url = new URL(request.url);
//     const params = url.searchParams;

//     const sort = params.get("sort") || "created_at_desc"; // Mặc định là mới nhất
...
//     let query = supabase
//       .from("products")
//       // === SỬA CÚ PHÁP SELECT: Dùng "ten_bang_ngoai(*)" ===
//       .select(
//         `
//         id, name, price, brand, condition, image_urls, created_at,
//         users ( username, avatar_url, is_verified )
//       `
//       ) // Yêu cầu join bảng 'users' (Supabase tự tìm khóa ngoại)
//       // ===========================================
//       .eq("status", "available");

//     // --- Xử lý Filter (Dùng tên bảng gốc 'users') ---
//     if (filterVerified) {
...
//     // --- Xử lý Sort (Dùng tên bảng gốc 'users') ---
//     if (sort === "verified_first") {
//       // Ưu tiên Verified LÊN ĐẦU
...
// // === HÀM POST (Giữ nguyên như cũ, dùng Service Key) ===
